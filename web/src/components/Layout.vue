...
export default {
  ...
  mounted () {
    let that = this
    this.bus.$on('triggerCollapse', function (func) {
      func()
      that.layoutStyle.marginLeft = that.isCollapsed ? '78px' : '200px'
    })
    this.layoutStyle.marginLeft = this.isCollapsed ? '78px' : '200px'
  }
}
</script>
