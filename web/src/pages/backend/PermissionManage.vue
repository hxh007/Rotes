...
export default {
  ...
  methods: {
    ...
    createPermissionOk (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          instance.post(process.env.API_ROOT + '/auth/permissions', {
    ...
    createPermissionCallback (response) {
      ...
      if (res.code === 0) {
        ...
        instance.get(process.env.API_ROOT + '/auth/permissions').then(this.loadAllPermissions)
        this.createPermissionFlag = false
      } else {
    ...
    editPermissionOk (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          instance.put(process.env.API_ROOT + '/auth/permissions/' + this.editFormValidate.id, {
    ...
    remove (id) {
      ...
      this.$Modal.confirm({
        ...
        content: '确认要删除该权限？',
        onOk: function () {
          instance.delete(process.env.API_ROOT + '/auth/permissions/' + id).then(function (response) {
            if (response.data.code === 0) {
              that.$Message.success('权限删除成功！')
              instance.get(process.env.API_ROOT + '/auth/permissions').then(that.loadAllPermissions)
            } else {
  ...
  mounted () {
    let that = this
    instance.get(process.env.API_ROOT + '/auth/permissions').then(this.loadAllPermissions)
    instance.get(process.env.API_ROOT + '/auth/departments').then(function (response) {
      let res = response.data
      if (res.code === 0) { // 查询成功
    ...
    instance.get(process.env.API_ROOT + '/auth/actiontypes').then(function (response) {
      let res = response.data
      if (res.code === 0) {
        res.data.forEach(function (item, index) {
