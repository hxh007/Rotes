  ...
  methods: {
    ...
    createOperationOk (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          instance.post(process.env.API_ROOT + '/auth/actiontypes', {
    ...
    createOperationCallback (response) {
      ...
      if (res.code === 0) {
        ...
        this.createOperationFlag = false
        instance.get(process.env.API_ROOT + '/auth/actiontypes').then(this.getAllOperations)
      }
    },
    remove (id) {
      let that = this
      this.$Modal.confirm({
        ...
        content: '确认要删除该操作？',
        onOk: function () {
          instance.delete(process.env.API_ROOT + '/auth/actiontypes/' + id).then(function (response) {
            if (response.data.code === 0) {
              that.$Message.success('操作删除成功！')
              instance.get(process.env.API_ROOT + '/auth/actiontypes').then(that.getAllOperations)
            } else {
